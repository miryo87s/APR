...
It is still unclear *what $\tau_{ols}$ identifies in the general case*

Another issue is that OLS seems to be *incompatible with the potential outcome framework*
...
[Overlap]{.blue}: less discussed, **not** assumed in the above three papers
...
$$
\tau_{ipw} = \mathbb{E}\left[\frac{Y_iD_i}{e(X_i)} - \frac{Y_i(1 - D_i)}{1 - e(X_i)}\right] = \tau_{ate}
$$
...
Next, I show the asymptotic equivalence of OLS and overlap IPW
...
::: {.callout-note #prp-wipw}
 ...
 (Asymptotic Equivalence of OLS and Overlap Weighted IPW)

Suppose the linear propensity assumption holds, the OLS estimator of $\tau_{ols}$
$$
\hat\tau_{ols} \xrightarrow{p} \tau_{overlap}
$$
...
$$
\hat\tau_{ols} = \frac{\sum_i Y_i\check{D_i}}{\sum_i \check{D}_i^2}, \quad \tau_{ols} = \frac{\mathbb{E}[Y_i\tilde{D}_i]}{\mathbb{E}[\tilde{D}_i^2]}
$$
where $\check{D}_i = D_i - X_i\hat{\beta}_e$ with $\hat{\beta}_e = (X'X)^{-1}X'D$, $\tilde{D}_i = D_i - X\beta_e$ with $\beta_e = \mathbb{E}[X'X]^{-1}\mathbb{E}[X'D]$. By the weak law of large numbers and Slusky's theorem, $\hat\tau_{ols} \xrightarrow{p} \tau_{ols}$. Then we have
$$\begin{aligned}
\tau_{ols} &= \frac{\mathbb{E}[Y_i(D_i - e(X_i))]}{\mathbb{E}[(D_i - e(X_i))^2]} = \frac{\mathbb{E}[Y_i(D_i - e(X_i))]}{\mathbb{E}[e(X_i)(1 - e(X_i))]} \\
...
- Often ignored by researchers because the linear regression of $e(x)$ is done in the background
  - In @aizer2016long, $31.2\%$ of the fitted propensities in the baseline model  is outside $[0, 1]$
  - In @sloczynski2022interpreting, $44.5\%$ of the fitted propensity score is negative

<br>

Next the impact of violating linear propensity score assumption is illustrated in Simulation and Application
