...
Binary and continuous treatment are discussed separately
...
<span style="font-size: 70%"> @rothenhausler2019incremental;@d2023nonparametric; @callaway2024difference; @borusyak2024negative <span>
...
-   $\lambda_i$ cannot preclude negative values
    -   Negative when $D_i = 1$ and $\tilde{D}_i < 0$, i.e. the
        fitted values of $D_i$ on $X_i$ exceeds 1
...
-   In practice, model-based can be less restrictive than design-based
    -   A suﬀicient and necessary condition for convex weights
       is having all fitted values of $D_i$ less than 1 for treated units              (negative values allowed)
    -   While design-based specification requires all fitted values in $[0, 1]$
    -   Will show in application
...
$$
\tau_{owfe} = \frac{\mathbb{E}[\lambda_{owfe, s}\tau_s]}{\mathbb{E}[\lambda_{owfe, s}]}, \quad \lambda_{owfe, s} = \frac{n_{1s}n_{0s}}{n_{1s} + n_{0s}}, \ \tau_s = \bar{Y}_{1s} - \bar{Y}_{0s}
$$
...
-   $\tilde{D}_{it}$ is negative for treated in pre-period, or control in
    post-period
...
-   First consider incremental effect by @kennedy2019nonparametric:
...
-   Will show $\mu'(D_i, X_i)$ identifies $\tau^0$ under model- and
    design-based specification
$$
(\partial/\partial d) \mathbb{E}[Y_i | D_i = d, X_i] = \mathbb{E}[Y_i'(d)|D_i = d, X_i] 
$$
-   Target is to show $\tau_{ls}$ is WADE
    $\tau_{wade} = \mathbb{E}[w(D_i, X_i)\mu'(D_i, X_i)]$ for some
    weight $w(d, x)$
-   The target $\tau_{wade}$ can be seen as a weighted index coefficient
...
Let $e(X_i) = \mathbb{E}[D_i |X_i]$.

$$
\mathbb{E}[D_i | Y_i(d), X_i] = e(X_i) \ \text{for all} \ d, \ \text{and} \ e \in \mathcal{G}.
$$
:::
...
-   Normal distribution $f(d|m, v)$ with mean $m = m(x)$ and variance $v = v(x)$: 
    -   $\omega(d, x) = \frac{Var(D_i | X_i)}{\mathbb{E}[Var(D_i | X_i)]}$
-   Gamma distribution $f(d|a, b)$ with shape $a = a(x)$ and rate $b = b(x)$: 
    -   $\omega(d, x) = \frac{Var(D_i | X_i)}{\mathbb{E}[Var(D_i | X_i)]} \frac{d}{m(x)}$
-   Beta distribution $f(d|a, b)$ with shape $a = a(x)$ and $b = b(x)$: 
    -   $\omega(d, x) = \frac{Var(D_i | X_i)}{\mathbb{E}[Var(D_i | X_i)]} \frac{d(1 - d)}{m(x)(1 - m(x)) - v(x)}$
...
-   When $D_i \in \{0, 1\}$
    -   $\mu(1, X_i) - \mu(0, X_i)$ identifies treatment effect under model- or design-based specifications.
-   Theorem 1 can be obtained by 
    -   $w(x) = \mathbb{E}[\alpha_w(D_i, X_i)D_i | X_i = x]$
    -   and $\mathbb{E}[w(X_i)(\mu(1, X_i) - \mu(0, X_i)]$ be the WATE

## LS is Optimally Efficient
...
## LS is Optimally Efficient {.scrollable}
...
i.  $\Vert e - \hat{e} \Vert = o_P(n^{-a/4})$ and
    $\Vert \rho - \hat{\rho} \Vert = o_P(n^{-b/4})$ where
    $a \geqslant 1, \ b\geqslant 0$ and $a + b \geqslant 2$.
ii.  $n^{-1}\sum_{i = 1}^n(D_i - \hat{e}(X_i)) > 0$ and
    $\mathbb{E}[D_i - e(X_i)] > 0$.
iii.  There exists a constant $0 < C < \infty$ such that
    $Var(D_i | X_i) < C$ and $Var(Y_i | X_i) < C$.

And assume that at least one of iv or v holds:

iv.  (Donsker condition) The quantities
    $(D_i - \hat{e}(X_i))(Y_i - \hat\rho(X_i))$ and
    $(D_i - \hat{e}(X_i))^2$ fall within a P-Donsker class with
    probability approaching $1$.
v.  (Cross-fitting) The sample used to estimate $\hat{e}(x)$ and
...
By Cauchy-Schwarz inequality and Assumption $i$, $R_n$ is
$o_P(n^{-1/2})$.
...
-   All four specifications are regarded as model- or design-based
...
![](fig/tbl2alt.png){fig-align="center" width="35%"}

## Application

![](fig/fig1.png){fig-align="center" width="75%"}

## Application

![](fig/fig2.png){fig-align="center" width="75%"}

## Application

<br>

Linear model-based specification can be less strict than linear design-based specification

::: {style="text-align: center;"}

![The Projection of $D_i$ on $X_i$ (Earnings in 1974)](fig/Dfit.png){fig-align="center"}
...
-   Negative weights are not necessarily a concern with model-based specifications
...
-   PLR is recommended over OLS
-   No concern with continuous treatment. For binary treatment:
    -   Weights can be estimated, and the distribution of weights should be examined
