...
- **A general causal interpretation of $\tau_{ols}$**
...
$$
  ...
  - This paper uses standard assumptions and the weights have similar properties
...
$$
  - $\tau_1$ and $\tau_2$ are biased estimators of ATEs even in randomized experiments
...
- @li2018balancing proposed **overlap-weighted IPW**
  - I show **OLS is overlap-weighted IPW under some conditions**
...
- Linear propensity score assumptions has been assumed in @aronow2016does; @abadie2020sampling; @sloczynski2022interpreting
  ...
  - This assumptions is inherently satisfied with saturated modela
...
## Properties of OLS (overlap weighted IPW)

$$
h(X_i) = \frac{e(X_i)(1 - e(X_i))}{\mathbb{E}[e(X_i)(1 - e(X_i))]}
$$

[Properties:]{.blue}

1. **Efficiency**: $Var(\hat\tau_{ols}) \leqslant Var(\hat\tau_{wipw})$ for all $h(X_i)$
2. **Robust to poor overlap**: 
    - Maximized with $e(x) = 0.5$
    - More weights when $e(x)$ is close to $0.5$, less weights when $e(x)$ away from $0.5$.
3. **Undesired weights**: Treated and control groups are weighted by probability of being assigned to the opposite group.
$$
\tau_{overlap} = \frac{\mathbb{E}[\color{red}(1 - e(X_i))\color{black}D_iY_i - \color{red}e(X_i)\color{black}(1 - D_i)Y_i]}{\mathbb{E}[e(X_i)(1 - e(X_i))]}
$$
    - @sloczynski2022interpreting weights have similar property
4. $\tau_{ols} \approx$ ATT when $e(x) \approx 0$ as $(1 - e(X_i), e(X_i)) \approx (1, \frac{e(x)}{1 - e(x)})$; $\tau_{ols} \approx$ ATU when $e(x) \approx 1$.
...
$$
- Doubly robust means that $\tau_{ob}$ is consistent of ATE if either $\mathbb{E}[D_i|X_i]$ or $\mathbb{E}[Y_i(d)|X_i]$ is correctly specified
...
Therefore,
$$\begin{aligned}
\tau_{ols} = \tau_{overlap} &+ \mathbb{E}\left[\frac{(D_i - e(X_i))(Y_i(1) - \mu_0(X_i))}{e(X_i)(1 - e(X_i))}\right] \\
&- \mathbb{E}\left[\frac{(D_i - e(X_i))e(X_i)(Y_i(1) - Y_i(0) + \mu_1(X_i) - \mu_0(X_i))}{e(X_i)(1 - e(X_i))}\right]
\end{aligned}$$
...
$$
\hat\tau^h_{HT} = \sum_{i = 1}^{N} h(X_i) \left(\frac{Y_i D_i}{\hat{e}(X_i)} - \frac{Y_i(1 - D_i)}{1 - \hat{e}(X_i)}\right) \bigg/ \sum_{i = 1}^{N} h(X_i)
$$
  - Hajek type estimator [@li2018balancing]
$$
\hat\tau^h_{Hajek} = \frac{\sum_{i = 1}^N Y_i D_i w_1(X_i)}{\sum_{i = 1}^N D_i w_1(X_i)} - \frac{\sum_{i = 1}^N Y_i (1 - D_i) w_0(X_i)}{\sum_{i = 1}^N (1 - D_i) w_0(X_i)}
$$
where $w_1(x) = h(x) / \hat{e}(x)$ and $w_0(x) = h(x) / (1 - \hat{e}(x))$. Both estimators are consistent for $\tau_{wipw}$ and they are asymptotically equivalent. 

- Proposition 4.1 shows that the two estimators are both consistent and asymptotically equivalent
- In finite sample, they are usually different
...
- Compare $\tau_{overlap}$ estimators of OLS/IPW with linear/logit $e(x)$, IPW-ATE
- The treated proportion is $1.1\%$
  - Also compare with IPW-ATT because overlap approximates ATT
...
- The interpretation of OLS estimand is overlap-weighted ATE
- OLS is not doubly robust and identification depends on specification of the propensity model
- When treated proportion $\rightarrow 0/\rightarrow 1$, OLS approximates ATT/ATU
